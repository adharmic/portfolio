...
export default function ExpandableCard({
  ...
  return (
    <div className={"flex flex-col " + className + (isOpen ? " h-full" : "")}>
      <div
        className={
          "p-2 flex items-center justify-between rounded-t 2xl:rounded-t-xl border border-foreground " +
          color +
          " " +
          className +
          (!isOpen ? " rounded 2xl:rounded-xl" : " rounded-t 2xl:rounded-t-xl")
        }
      >
        <h1 className="font-bold 2xl:text-4xl">{heading}</h1>
        <Minimize callback={toggleOpen}></Minimize>
      </div>
      <div
        className={
